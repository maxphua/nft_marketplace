...
// control + click on ercstorage to view code
// private means cannot call outside of the current class
// public means available from the client application
// require means condition or requirement
// payable means access a function to receive ehereum
// view means it's not doing any transaction work
// memory means some data we want to return

// Creating our contract ->Inherited from ERC721URIStorage
contract NFTMarketplace is ERC721URIStorage {
    using Counters for Counters.Counter;

    // when we mint the first token, it will get a value of 0, 2nd one is one, and so on, we need to have some kind of id
    Counters.Counter private _tokenIds; // create token ID
    ...
    // have an event for trigger when a market item is created.
    ...
    /* Create or Mints a token and lists it in the marketplace */
    // converting image into no fungible token
        ...
        returns (uint256)
    {
        ...
        // mint the token with, the id, the person who created it
        _mint(msg.sender, newTokenId);

        // make the token unique
        _setTokenURI(newTokenId, tokenURI);

        // take the minted token and listed on the marketplace
    ...
    // created the token and put it into the marketplace
    function createMarketItem(uint256 tokenId, uint256 price) private {
        // require a certain CONDITION, in this case price greater than 0
        require(price > 0, "Price must be at least i");
    ...
    function createMarketSale(uint256 tokenId) public payable {
        ...
        require(
            msg.value == price,
            "Please submit the asking price in order to complete the perchase"
        );

        idToMarketItem[tokenId].owner = payable(msg.sender);
        idToMarketItem[tokenId].sold = true;
        idToMarketItem[tokenId].seller = payable(address(0)); // address 0 means it belongs to the marketplace
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        // get all the market item that have the address 0
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            // only looking for the item belongs to the marketplace not the specific people
            //if the owner is equal to this contract, this is the id we want to interact with
